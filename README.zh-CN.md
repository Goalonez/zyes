...
**面向 AI 编码代理的文档驱动工作流——用一份 Markdown 文档规划，跨会话、跨 agent 执行。**
...
长任务最容易崩在这几个时刻：上下文被压缩、会话结束、或者你换了个 agent——计划只存在于对话里，下一个 agent 只能从零开始。

**Zyes 用一份本地 Markdown 文档解决这个问题。** 需求、关键决策、执行步骤、进度和验证结果全都落在同一份文档里，任何 agent 打开即可接手。没有数据库、没有状态机、没有脚本——文档本身就是状态。
...
- **为跨 agent 而设计**——把规划文档交给任意 agent，只需一条约定：*“读规划文档，执行第一个未勾选步骤，勾选并追加进度日志。”*
...
`z-implement` 一次只推进**一个**未完成步骤，然后勾选 checkbox 并在进度日志追加一行（改了什么、验证结果、下一步）。所有步骤完成且验收满足时，顺势收尾：把 `status` 置为 `done` 并移入 `plans/done/`。
...
Zyes 适合跨会话、多步骤、值得持久化的工作。对于一次性小改动、纯问答、或 agent 一轮就能搞定的事——跳过它。这些 skill 被刻意收敛过，遇到这类情况会保持安静。
